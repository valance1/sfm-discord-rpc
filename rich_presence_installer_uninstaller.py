...
# WRITTEN BY VALANCE STEAM ID: theziker
# 13/01/2022 - v1.0
absoluteimport = """
...
#end discord
'''
starterConf = u'[True, True, True, True, False, True, 0, "I am using SFM! ", 0]'
...
# https://stackoverflow.com/questions/28440722/delete-all-text-in-a-file-between-two-strings-without-importing-any-modules
# props to this answer bcuz i'm too lazy to develop this last bit at 4AM
def excise(filename, start, end):
    ...
    text = line[:i] + line[j+len(end):]

    out = open(filename, 'w')
    out.write(text)

def installfuture(filename):
    with open('usermod\\scripts\\sfm\\sfm_init.py') as f:
        lines = f.readlines()
        with open('usermod\\scripts\\sfm\\sfm_init.py', 'w') as arquivo:             
            arquivo.write(unicode(str("from __future__ import absolute_import")))
            arquivo.writelines(absoluteimport)
passar = 1

with open('usermod\\scripts\\sfm\\sfm_init.py', 'r') as file: # dont install if already installed
    for line in file.readlines():
        passar = 1
        if ("#discord" in line):
    
            excise(u"usermod\\scripts\\sfm\\sfm_init.py", "#discord", "#end discord")
            MessageBox("Discord RPC Uninstalled")
            passar = 0 # just to make sure
            break
        else:
            pass
...
if passar == 1:
    ...
    with open("usermod\\scripts\\sfm\\sfm_init.py", 'r') as file:
        for line in file.readlines():
            passar = 1
            if ("from __future__ import absolute_import" not in line):
                installfuture(u"usermod\\scripts\\sfm\\sfm_init.py")
                break
            else:
                break
                
    
    with open('usermod\\scripts\\sfm\\sfm_init.py', 'a') as file:
        file.write(unicode(str(script), "utf-8"))
        MessageBox("Discord RPC Installed")


    with open('usermod\\rpcConfig.cfg', 'w') as file:
        file.write(unicode(str(starterConf), "utf-8"))
        MessageBox("Discord RPC Configurations written")
